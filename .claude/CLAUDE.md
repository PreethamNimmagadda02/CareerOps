...
## Codebase Intelligence for CarrerOps (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-19 (commit a479b50). Confidence: 100%.
...
### Architecture
repo is a centralized platform for managing professional profiles and CV data: it ingests raw user input via a web interface, persists structured data into DynamoDB, and orchestrates background processing tasks to generate and serve profile artifacts. The system functions as a full-stack application that bridges a TypeScript-based frontend with a robust serverless backend, ensuring data integrity and session management across the lifecycle of a user's professional record. The repository maintains a healthy balance of complexity, with no files currently identified as high-churn/high-complexity hotspots. The core logic is primarily driven by the src/lib directory, which provides stable, reusable utilities for database operations and logging.
...
- `web/components/dashboard.tsx` — 14 commits/90d (100.0th %ile)
- `src/cli/scan.ts` — 20 commits/90d (99.4th %ile)
- `web/components/profile-view.tsx` — 7 commits/90d (98.8th %ile)
- `.repowise/.update.log` — 6 commits/90d (98.2th %ile)
- `.repowise/knowledge-graph.json` — 5 commits/90d (97.5th %ile)

### Code health
Three co-equal signals: defect risk 8.86/10 avg, hotspot health 6.68/10 (stable), worst `web/components/launch-pad.tsx` at 2.77/10 · maintainability 9.63/10 · performance risk 12 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `src/lib/cv-store.ts` — untested hotspot — impact −2.0
- `src/lib/profile-store.ts` — untested hotspot — impact −2.0
- `web/components/pipeline-provider.tsx` — change entropy — impact −1.8
- `web/components/dashboard.tsx` — change entropy — impact −1.3
- `web/lib/pipeline.ts` — churn risk — impact −1.3
