...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-18 (commit 63dcc0f). Confidence: 100%.
...
### Architecture
repo is a centralized profile and career data management platform that ingests raw user professional data, processes it through a structured persistence layer, and surfaces it via a secure, session-managed web interface. The system orchestrates the flow of data from external inputs into a DynamoDB-backed store, utilizing a background worker architecture to handle asynchronous tasks and a TypeScript-based frontend to provide a reactive user experience.
...
- `web/components/dashboard.tsx` — 14 commits/90d (99.3th %ile)
- `src/cli/scan.ts` — 20 commits/90d (98.6th %ile)
- `web/components/profile-view.tsx` — 7 commits/90d (97.9th %ile)
- `web/components/pipeline-provider.tsx` — 9 commits/90d (97.3th %ile)
- `src/cli/evaluate.ts` — 14 commits/90d (96.6th %ile)

### Code health
Three co-equal signals: defect risk 8.67/10 avg, hotspot health 6.19/10 (stable), worst `web/components/pipeline-provider.tsx` at 2.74/10 · maintainability 9.61/10 · performance risk 12 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `src/lib/cv-store.ts` — untested hotspot — impact −2.0
- `src/lib/profile-store.ts` — untested hotspot — impact −2.0
- `web/components/dashboard.tsx` — untested hotspot — impact −2.0
- `web/lib/pipeline.ts` — untested hotspot — impact −2.0
- `web/components/pipeline-provider.tsx` — change entropy — impact −1.4
