...
*   **The Scanner (`index.mjs`)**: You run the scanner using `npm start`. It parses `portals.yml` and uses a hybrid extraction approach:
...
*   **The Evaluator (`evaluate-agent.mjs`)**: You trigger this via `npm run evaluate`.
...
graph TD
    subgraph Ingestion
        A[portals.yml] --> B(Scanner index.mjs)
        B -->|Greenhouse API/Playwright| C[(data/applications.md)]
    end
    
    subgraph AI Evaluation Loop
        C -->|Unscored Jobs| D(Evaluator evaluate-agent.mjs)
