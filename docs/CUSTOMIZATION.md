...
Career-ops can integrate with external systems via Claude Code hooks or Copilot CLI/coding-agent hooks.
...
```json
{
  ...
  "hooks": {
    "sessionStart": [
      {
        "type": "command",
        "bash": "node update-system.mjs check || true",
        "powershell": "node update-system.mjs check; exit 0",
        "cwd": ".",
        "timeoutSec": 10
      }
    ]
...
Save Copilot hooks in `.github/hooks/*.json`.
