...
// Canonical status mapping (English — matching states.yml)
function normalizeStatus(raw) {
  ...
  if (/^(rechazado|rejected)\s+\d{4}/i.test(s)) return { status: 'Rejected' };

  // Aplicado/Applied with date → Applied (strip date)
  if (/^(aplicado|applied)\s+\d{4}/i.test(s)) return { status: 'Applied' };
  ...
  // Spanish aliases → English canonical
  if (['evaluada'].includes(lower)) return { status: 'Evaluated' };
  if (['aplicado', 'enviada', 'aplicada', 'sent'].includes(lower)) return { status: 'Applied' };
