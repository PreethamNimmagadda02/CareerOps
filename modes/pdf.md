...
14. Run: `npm run pdf -- /tmp/cv-candidate-{company}.html output/cv-candidate-{company}-{YYYY-MM-DD}.pdf --format={letter|a4}`
...
- No critical info in the PDF headers/footers (ATS ignores them)
...
**NEVER add skills the candidate doesn't have. Only rephrase real experience using the exact vocabulary from the JD.**
...
Update the tracker if the role is already registered: change PDF from ❌ to ✅.
